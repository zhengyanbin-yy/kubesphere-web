...
const renderIndex = async (ctx, params) => {
  ...
  const rootClass = []
  let hides = query['hide']
  const hideall =
    undefined !== query['hide-all'] ||
    query['consoletoken'] ||
    ctx.cookies.get('consoleToken')
  if (hideall || hides) {
    hides = !hideall ? hides.split(',') : ''
    ;(hideall || hides.indexOf('header') > -1) &&
      rootClass.push('x-hide-header')
    ;(hideall || hides.indexOf('side') > -1) && rootClass.push('x-hide-side')
    ;(hideall || hides.indexOf('title') > -1) && rootClass.push('x-hide-title')
  }
...
const renderViewErr = async (ctx, err) => {
  const query = ctx.query || {}
  let consoletoken = query.consoletoken || ctx.cookies.get('consoleToken')
  ...
  if (err) {
    if (err.code === 401 || err.code === 403 || err.status === 401) {
      if (isValidReferer(ctx.path)) {
        const consoletokenParam = !consoletoken ? '' : `${consoletoken}&`
        ctx.redirect(`/login?${consoletokenParam}referer=${ctx.path}`)
      } else {
        const consoletokenParam = !consoletoken ? '' : `?${consoletoken}`
        ctx.redirect(`/login${consoletokenParam}`)
      }
