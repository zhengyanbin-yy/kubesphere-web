...
const getAccount = async token => {
  let getAccountUrl = serverConfig.consoleServer.getAccountUrl
  if (!getAccountUrl) {
...
const consoleLogin = async ctx => {
  let user = null

  const query = ctx.query || {}
  const consoleToken = query.consoletoken || ctx.cookies.get('consoleToken')
  if (consoleToken) {
    const account = await getAccount(consoleToken)
    const params = {}
    /* params.encrypt = 'MDAxMTAwMTAxMDAw@`]QLaenjiSA' */
    params.username = account.row.account
    params.password = decryptPassword(
      account.row.password,
      account.row.managementName + account.row.account
    )
    ...
    try {
      ...
      console.error(error)
      ctx.cookies.set('consoleToken', null)
      return
    }

    ctx.cookies.set('consoleToken', consoleToken)
...
const getCurrentUser = async ctx => {
  ...
  const consoleToken = ctx.cookies.get('consoleToken')
  let userDetailInfo
  const query = ctx.query || {}

  if (!token || (query.consoletoken && query.consoletoken !== consoleToken)) {
    const user = await consoleLogin(ctx)
    user && (token = user.token)
  } else {
    try {
      userDetailInfo = await getUserDetail(token)
    } catch (err) {
      if (err) {
  ...
  const [userDetail, workspaces] = await Promise.all([
    userDetailInfo || getUserDetail(token),
    getWorkspaces(token),
  ])
